...
export function renderMetroMapHTML() {
  ...
  <div class="nb-card p-4 text-start shadow-sm border-0" style="position: relative; z-index: 1; background: var(--bs-card-bg); border-radius: 16px;">
    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
      <div>
        <h5 class="fw-bold mb-1 text-body"><i class="bi bi-map-fill text-primary me-2"></i>Interactive Namma Metro Network Map</h5>
        <p class="text-secondary mb-0" style="font-size:0.85rem;">Live geographic map with full <i class="bi bi-circle-fill me-1" style="color: #16a34a;"></i> Green (32), <i class="bi bi-circle-fill me-1" style="color: #9333ea;"></i> Purple (36), and <i class="bi bi-circle-fill me-1" style="color: #eab308;"></i> Yellow (16) corridors & station popups.</p>
      </div>
      <div class="d-flex align-items-center gap-2 flex-wrap">
        <span class="badge text-white px-3 py-2 rounded-pill" style="background:#9333ea;"><i class="bi bi-circle-fill me-1" style="color: #9333ea;"></i> Purple (36)</span>
        <span class="badge text-white px-3 py-2 rounded-pill" style="background:#16a34a;"><i class="bi bi-circle-fill me-1" style="color: #16a34a;"></i> Green (32)</span>
        <span class="badge text-dark px-3 py-2 rounded-pill" style="background:#ffc61a;"><i class="bi bi-circle-fill me-1" style="color: #eab308;"></i> Yellow (16)</span>
      </div>
    </div>

    <!-- Map Canvas Container -->
    <div id="nammaMetroMapCanvas" class="rounded-3 border overflow-hidden shadow-2xs position-relative" style="width: 100%; height: 520px; z-index: 1;"></div>
  </div>`;
}
...
export function initMetroLeafletMap() {
  ...
  // Centered over Bengaluru
  const map = L.map('nammaMetroMapCanvas', {
    center: [12.9616, 77.5846],
    zoom: 11,
    zoomControl: true
  });
  ...
  // Sort Green line stations to include Majestic in geographical sequence between Sampige Road & Chickpet
  const sortedGreen = [];
  for (const s of stationsData) {
    if (s.line === 'green' && s.lat && s.lng) {
      sortedGreen.push(s);
      if (s.id === 'sampige-road' || s.id === 'mantri-square-sampige-road') {
        const maj = stationsData.find(st => st.id === 'majestic');
        if (maj && !sortedGreen.some(st => st.id === 'majestic')) {
          sortedGreen.push(maj);
        }
      }
    }
  }
  ...
  // Add Yellow Line Polyline (Thick Yellow Corridor)
  ...
  // Add Green Line Polyline (Thick Green Corridor)
  ...
  // Add Purple Line Polyline (Thick Purple Corridor)
  ...
  // Add Markers for all 83 stations
  stationsData.forEach(s => {
    ...
    const radius = isInterchange ? 9 : 6;
